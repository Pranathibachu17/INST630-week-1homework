...
document.addEventListener('DOMContentLoaded', (event) => {
   ...
   filterButton.addEventListener('click', () => {
    // Step 1: filter creates a NEW array with only matching items
    const cheapRestaurants = restaurants.filter((restaurant) => {
        return restaurant.priceRange === '$' || restaurant.priceRange === '$$';
    });

    // Step 2: clear and display the filtered results
   ...
   mapButton.addEventListener('click', () => {
    // Step 1: map transforms each restaurant into just its name
    const names = restaurants.map((restaurant) => {
        return restaurant.name;
    });

    // Step 2: display as a simple list
   ...
   findButton.addEventListener('click', () => {
    // Step 1: find returns the FIRST restaurant matching the condition
    const bestRestaurant = restaurants.find((restaurant) => {
        return restaurant.rating === 4.8;
    });

    // Step 2: check it exists, then display it
    if (bestRestaurant) {
        foundItem.innerHTML = `
            <div class="found-restaurant">
                ...
                <div>${bestRestaurant.neighborhood}</div>
            </div>
        `
    }
